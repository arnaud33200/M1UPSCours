...
/* Creation des elements gstreamer */
    ...
    videoconv = gst_element_factory_make("theoradec", "videoconv");
    ...
    videobalance = gst_element_factory_make ("videobalance", "video-output");
...
/* on configurer le nom du fichier a l'element source */
    // definir les propriétés d'un plugin

    g_object_set (G_OBJECT (source), "location", argv[1], NULL);
    g_object_set (G_OBJECT (volume), "volume", 0.75, NULL);
    g_object_set (G_OBJECT (videobalance), "saturation", (gdouble)0, NULL);
...
/* file-source | ogg-demuxer | vorbis-decoder | converter | alsa-output */
    ...
    gst_bin_add_many (GST_BIN (pipeline), pipeline, source, demuxer, queueaudio, queuevideo, audiodecoder, videodecoder, audiosink, videosink, volume, videobalance, NULL);
...
/* file-source -> ogg-demuxer ~> vorbis-decoder -> converter -> alsa-output */
    ...
    gst_element_link_many (queueaudio, audiodecoder, volume, audiosink, NULL);
    gst_element_link_many (queuevideo, videodecoder, videobalance, videosink, NULL);
