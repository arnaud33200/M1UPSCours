...
public:
	...
	*/
	Color evaluate (const Vector3D& to_eye_, const Vector3D& to_light_) const {
		...
		case BLINN_PHONG:
		{
			...
			Vector3D half = (to_eye_ + to_light_).normalize();
                       ...
                       return m_kd + m_ks * (powf ((half.dot (m_normal)), m_shininess) * (m_shininess * 4) / (8.f * M_PI));
                       //return m_kd + m_ks * powf ((half.dot (m_normal)), m_shininess);
			break;
		}
		case PURE_REFL:
	...
	bool has_reflection () const {
		return (m_model == FRESNEL) || (m_model == PURE_REFL);
	}
